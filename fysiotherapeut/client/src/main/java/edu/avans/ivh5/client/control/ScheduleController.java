...
public class ScheduleController implements ActionListener, KeyListener, MouseListener {
    ...
    public void setTableData() throws ParseException {
        ...
        repaintTable();
        // ALl available data is inserted, now add an actionlistener to the empty cells.
        for(int f = 0; f < model.getColumnCount(); f++){
            System.out.println("test");
            for(int g = 0; g < model.getRowCount(); g++){
                if(model.getValueAt(g, f) == null){
                    System.out.println("testing if empty");
                    //model.setValueAt("empty", g, f);
                    // add actionlistener to cell.
